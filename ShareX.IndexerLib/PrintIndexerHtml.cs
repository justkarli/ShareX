...
{
    public class PrintIndexerHtml : PrintIndexerOutput
    {
        protected override string PrintFolderInfo()
        {
            ...
            html_body_builder.AppendLine(HtmlHelper.Tag("title", "Index for " + Path.GetFileName(Indexer.FolderPath)));
            html_body_builder.AppendLine(HtmlHelper.GetCssStyle(IndexerSettings.CssFilePath));
            ...
            if (IndexerSettings.AddValidationIcons) 
        ...
        protected override string GetFolderNameRow(DirectoryFileInfo directory_info, int level = 0)
        {
            ...
            {
                folderInfoText = directory_info.DataSize.ToSizeString(IndexerSettings.BinaryUnits) + " (";
        ...
        {
            string size = " " + HtmlHelper.Tag("span", fi.Length.ToSizeString(IndexerSettings.BinaryUnits), "", "class=\"filesize\"");
